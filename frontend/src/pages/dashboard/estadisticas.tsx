...
const colorNivel: Record<string, string> = {
  Excelente: 'bg-emerald-100 text-emerald-700 border border-emerald-300',
  Bueno:     'bg-blue-100 text-blue-700 border border-blue-300',
  Regular:   'bg-amber-100 text-amber-700 border border-amber-300',
  Malo:      'bg-red-100 text-red-700 border border-red-300',
};
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 flex items-start gap-4">
          <div className="w-12 h-12 rounded-xl bg-[#6B0F0F]/10 flex items-center justify-center shrink-0">
            <svg className="w-6 h-6 text-[#6B0F0F]" fill="none" stroke="currentColor" strokeWidth={1.5} viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" d="M3 7h18M3 12h18M3 17h18" />
            </svg>
          </div>
          <div>
            <p className="text-xs text-gray-400 uppercase tracking-wide font-medium">Piezas registradas</p>
            <p className="text-4xl font-bold text-gray-900 mt-0.5 leading-none">{stats.totalActivas}</p>
            <p className="text-xs text-gray-500 mt-1">en el inventario activo</p>
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 flex items-start gap-4">
          ...
          <div>
            <p className="text-xs text-gray-400 uppercase tracking-wide font-medium">Movimientos este mes</p>
            <p className="text-4xl font-bold text-blue-600 mt-0.5 leading-none">{stats.movimientosMes}</p>
            <p className="text-xs text-gray-500 mt-1">traslados y préstamos</p>
          </div>
        </div>

        {/* Mal estado */}
        <div className={`rounded-2xl shadow-sm border p-5 flex items-start gap-4 ${stats.malEstado > 0 ? 'bg-red-50 border-red-200' : 'bg-white border-gray-100'}`}>
          <div className={`w-12 h-12 rounded-xl flex items-center justify-center shrink-0 ${stats.malEstado > 0 ? 'bg-red-100' : 'bg-gray-100'}`}>
            <svg className={`w-6 h-6 ${stats.malEstado > 0 ? 'text-red-500' : 'text-gray-400'}`} fill="none" stroke="currentColor" strokeWidth={1.5} viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" d="M12 9v3.75m-9.303 3.376c-.866 1.5.217 3.374 1.948 3.374h14.71c1.73 0 2.813-1.874 1.948-3.374L13.949 3.378c-.866-1.5-3.032-1.5-3.898 0L2.697 16.126zM12 15.75h.007v.008H12v-.008z" />
            </svg>
          </div>
          <div>
            <p className="text-xs text-gray-400 uppercase tracking-wide font-medium">Piezas en mal estado</p>
            <p className={`text-4xl font-bold mt-0.5 leading-none ${stats.malEstado > 0 ? 'text-red-600' : 'text-gray-900'}`}>{stats.malEstado}</p>
            <p className="text-xs text-gray-500 mt-1">requieren intervención urgente</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="text-sm font-bold text-gray-800">Estado de conservación</h3>
          </div>
              ...
              return (
                <div key={nivel}>
                  <div className="flex justify-between mb-1">
                    ...
                    <span className="text-xs font-bold text-gray-700">{total} <span className="font-normal text-gray-400">({pct}%)</span></span>
                  </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
          <div className="flex items-center gap-2 mb-4">
            <div className="w-1 h-5 rounded-full bg-[#6B0F0F]" />
            <h3 className="text-sm font-bold text-gray-800">Piezas por categoría</h3>
          </div>
          <div className="space-y-3">
              ...
              <tbody className="divide-y divide-gray-50">
                {stats.alertas.map((a) => (
                  <tr key={a.idPieza} className="hover:bg-gray-50 transition-colors">
                    <td className="px-5 py-2.5 font-mono text-xs text-gray-500">{a.codigoInventario}</td>
                    <td className="px-4 py-2.5 font-medium text-gray-800">{a.nombre}</td>
                    <td className="px-4 py-2.5">
                      <span className={`text-xs px-2.5 py-0.5 rounded-full font-medium ${colorNivel[a.estadoConservacion] ?? ''}`}>
                        {a.estadoConservacion}
                      </span>
                    </td>
                    <td className="px-4 py-2.5 text-xs text-gray-500">{a.ubicacion?.nombre ?? '—'}</td>

// categorias/editar/index.tsx — HU-006: editar categoría
import { useEffect, useState } from 'react';
import { useNavigate, useParams } from 'react-router-dom';
import { listCategorias, updateCategoria, CategoriaOpcion } from '../../../api/categorias';
...
export default function EditarCategoriaPage() {
  ...
  useEffect(() => {
    listCategorias().then((data: CategoriaOpcion[]) => {
      const c = data.find((x) => x.idCategoria === id);
      if (c) setForm({ nombre: c.nombre, descripcion: c.descripcion ?? '' });
    });
  }, [id]);
      ...
      {exito && (
        <div className="fixed inset-0 flex items-center justify-center bg-black/30 z-50">
          <div className="bg-white rounded-2xl shadow-2xl px-10 py-8 flex flex-col items-center gap-3">
            ...
            <p className="text-lg font-semibold text-gray-800">¡Categoría actualizada!</p>
